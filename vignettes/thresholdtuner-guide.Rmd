...
*Various graph theoretical strategies have been proposed in an effort to determine a most appropriate threshold, but no overall consensus has been reached as to which techniques are best suited to various graph classes* [@DISSERTATION]."

This  package is meant to wrap and adapt the functionality from an existing C++ command line tool for the same purpose. This original package was written by Dr. Carissa Bleker and Dr. Stephen Grady, and it provides programs for graph analysis, visualization, and thresholding. The command line package's code, shell script tools, and Jupyter notebooks can be found on [Github (https://github.com/carissableker/thresholding)](https://github.com/carissableker/thresholding). These tools were adapted to the R environment and was written using the Rcpp library [@RCPP] to easily port the C++ logic to R code. The original C++ package bundled and statically linked against the igraph C library [@IGRAPH] upon installation. This R package also includes the igraph dependency and depends upon *cmake* and *GNU make* to build (these dependencies are included in R Tools for Windows and can be easily installed with package managers in *Nix environments).   
...
The following tutorial will use the "*HumanCellCycleSubset.ncol*" graph file located in the package's `extdata` directory. This file had to remove a significant number of edges to fit within the suggested 1 MB requirement, so some analyses are different for the full file. The contents of the entire data sets are linked in the Github repo for the original package (see [Introduction](#introduction)). 

Each row in this edge list contains two genes (vertices) that are connected by a Pearson Correlation value (edge weight) representing the "strength" of the gene-gene correlation.
...
## `edge_hist()`: visualize graph edge weight distribution
  Using `edge_hist` will provide a simple way to display a histogram of
the edge weight distribution of an input graph file (see [Introduction](#introduction) for more details about the *.ncol* format). 
`edge_hist` takes in the path of the input graph file and several optional parameters that adjust the width of the histogram bins, additional control for the separator of the *.ncol* graph input file, and giving a title to the output plot.
...
## `analysis()`: run thresholding method calculations and analyses
  `analysis` can perform an assortment of thresholding algorithms during an iterative thresholding loop. These algorithms, or methods, are further described in [@JCB] and [@DISSERTATION]. The list of implemented methods can be found by running `?analysis`. 
`analysis` writes the output of its operations to files ending with `.iterative.txt` or `.statistical.errors.txt` to match the behavior of the existing C++ package. *`analysis` will return the file path prefix that will later be used by `get_results` so it is recommended to store the output inside a variable for later use in script workflows.*

  Like other functions in the package, `analysis` will take the path of the input graph file as a mandatory argument. All other arguments are optional, but core functionality will greatly be impacted by `methods`, `lower`, `upper`, `increment`, and `num_samples` (only for the Significance and Power Calculations method). 
As `analysis` forms a large portion of ThresholdTuner's utility, more documentation about the additional parameters would not fit into a vignette guide. Run `?analysis` for more details on the optional parameters and the cited papers for more technical details. 
...
starting_thresh <- 0.7   # choose lower bound value for thresholding loop to start from 
...
`get_results` can also output the raw R dataframes from `analysis` and `show()` a plot of where the promising thresholds overlayed over the graph's vertices and edges through the thresholding process (see [plot_t_vs_ev](#plot_t_vs_ev) for more details on this plot. The option to plot in `get_results` will not return the `ggplot2` object that `plot_t_vs_ev` does). 
...
## `plot_t_vs_ev()`: plot thresholding vs. edges and vertices {#plot_t_vs_ev}
  To get a clearer picture of where the recommended threshold values intersect with the input graph's vertices and edges, `plot_t_vs_ev` plots vertical lines across the edge and vertex curves from the thresholding loop. This function simply takes the prefix returned from `analysis` to make the plot. The output plot will only contain the **iterative** methods from the `analysis` results (no Significance and Power recommended values will be included on the plot.)
...
Now that there are several thresholds computed, a threshold can now be applied to the input graph.
`threshold` will ensure that a new graph file is created from an input graph file (the input graph will not altered in the process). The new  graph will be formed according to one of several thresholding methods that can be applied to the input graph:
...
The output graph file will have the edges removed from the input graph according to the `thresh` value and the string `method` selected. 
...
ThresholdTuner provides graph thresholding utilities that can be incorporated into a standard R scripting workflow. The package includes functions  to identify ideal thresholds using analytical graph algorithms, visualize graph properties and iterative threshold results, and output thresholded graphs. ThresholdTuner used the Rcpp R library [@RCPP], igraph C library [@IGRAPH], and the ALGLIB library [@ALGLIB], which was used for statistical distributions.  And as restated from [@JCB]: *"A thresholding repository such as this has not, to the best of our knowledge, previously been curated and released to the scientific community."*
